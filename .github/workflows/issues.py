...
def comment_issue(issue_number: int, comment: str) -> None:
    ...
    requests.post(f"{API}repos/{REPO}/issues/{issue_number}/comments", data=payload, headers=headers)
...
def main() -> None:
    ...
    if not check_if_follows_template(issue_number):
        comment = """This Issue is getting close because it does not follow any given template.
Try reposting the issue following one of the given templates."""
        ...
        comment = """Thanks for submitting your issue.
An assignee will be with you very soon."""
        comment_issue(issue_number, comment)
        add_assignees(issue_number, assignees)

    else:
        comment = """Thanks for submitting your issue.
Unfortunately we could not find an aviable assignee for you.
